  ...
  >
    <template v-slot:item="{ item, index }">
      <tr>
        ...
        <td class="border border-gray-300">
          <v-chip :color="item.is_active ? 'success' : 'error'" dark small>
            {{ item.is_active ? 'Active' : 'Inactive' }}
          </v-chip>
        </td>
        <td class="border border-gray-300">
          <div class="d-flex justify-end">
            <v-btn icon="mdi-pencil" variant="text" @click="editUser(item)"></v-btn>
            <v-btn icon="mdi-delete" variant="text" @click="deleteUser(item)"></v-btn>
          </div>
        </td>
      </tr>
    </template>
...
const emits = defineEmits(['update-options', 'edit-user', 'delete-user', 'page-changed', 'items-per-page-changed', 'show-details']);
